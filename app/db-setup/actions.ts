...
// Create a connection for this specific action
...
export async function seedDatabase() {
  ...
  try {
    ...
    if ((existingChannels[0] as any).count > 0) {
      ...
      redirect("/db-setup/success?message=Database already seeded")
    }
    ...
    redirect("/db-setup/success")
  } catch (error) {
    console.error("Database setup error:", error)
    if (connection) await connection.end()
    redirect(`/db-setup/error?message=${encodeURIComponent(String(error))}`)
  }
}
